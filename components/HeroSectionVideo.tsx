            ...
            <div className="relative z-10 h-full max-w-[1920px] mx-auto px-12">
                ...
                <div className="absolute top-28 left-0 right-0 flex justify-between px-12 pointer-events-none">
                    <div className="flex items-center gap-3 bg-white/10 backdrop-blur-xl border border-white/20 rounded-full px-5 py-2.5">
                        ...
                        <span className="font-space font-bold text-xs tracking-[0.2em]">{restaurantData.hero.badge}</span>
                    </div>
                    <div className="flex items-center gap-4 bg-black/40 backdrop-blur-md rounded-full px-5 py-2 border border-white/10">
                        <div className="text-right">
                            <p className="text-xs font-bold font-space">{restaurantData.hero.chefName}</p>
                            <p className="text-[10px] text-white/50 uppercase">{restaurantData.hero.chefTitle}</p>
                        </div>
                ...
                <div className="absolute top-[40%] left-[20%] hidden lg:flex flex-col items-center gap-2 pointer-events-none">
                    <div className="bg-black/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/20 flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold font-space">{restaurantData.hero.ingredientName}</span>
                    </div>
                    <div className="h-24 w-[1px] bg-gradient-to-b from-blue-500/50 to-transparent dashed-line" />
                </div>

                {/* Bottom Right Culinary Metrics */}
                <div className="absolute bottom-12 right-12 bg-black/80 backdrop-blur-3xl border border-white/10 rounded-[2rem] p-8 w-80 shadow-2xl">
                    <div className="flex justify-between items-start mb-6">
                        ...
                        <div className="text-right">
                            <p className="text-[10px] font-space text-white/40 tracking-widest">{restaurantData.hero.tempMetric.label}</p>
                            <p className="text-5xl font-bold font-space">{restaurantData.hero.tempMetric.value}</p>
                            <p className="text-xs text-blue-400 font-bold tracking-widest mt-1">{restaurantData.hero.tempMetric.status}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="absolute bottom-12 left-12 max-w-5xl pointer-events-none">
                    <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8 }}>
                        <div className="relative">
                            <h1 className="text-[9rem] font-bold text-transparent bg-clip-text bg-gradient-to-b from-white via-white/80 to-white/40 leading-[0.8] tracking-tighter drop-shadow-2xl">{restaurantData.hero.title}</h1>
                        </div>
                        <div className="mt-8 pl-6 border-l-2 border-white/40 pointer-events-auto">
                            <p className="text-xl font-rajdhani text-gray-200">{restaurantData.hero.subtitle}</p>
                            <div className="flex gap-4 mt-6">
                                <button className="px-8 py-3 bg-white text-black font-bold font-space text-sm tracking-widest rounded transition hover:scale-105">{restaurantData.hero.cta.primary}</button>
                                <button className="px-8 py-3 border border-white/30 text-white font-bold font-space text-sm tracking-widest rounded backdrop-blur-md hover:bg-white/10">{restaurantData.hero.cta.secondary}</button>
                            </div>
                        </div>
                    </motion.div>
                </div>
